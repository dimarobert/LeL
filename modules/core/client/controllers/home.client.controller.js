...
angular.module('core').controller('HomeController', ['$scope', '$interval', 'Authentication', 'CoursesService', 'Top15Service',
  function ($scope, $interval, Authentication, CoursesService, Top15Service) {
    ...
    (function () {
      if ($scope.authentication.user) {
        updateTop15();
        //start the timeout
        var topUpdate = $interval(function () {
          updateTop15();
        }, 5000);

        $scope.$on('$destroy', function () {
          if (topUpdate) {
            $interval.cancel(topUpdate);
          }
        });
      }
